...
| 7 | `b6b1c330a` AKS plugin command support | **FOLD / CMD** | Fold into #106; replace plugin-name branches with broker capabilities. |
| 8 | `920dd7a8b` Frontend kubectl integration | **FOLD / CMD** | Fold into #106; consume the supported browser command API from the plugin. |
| 9 | `290767161` Update `productName` | **FOLD / IDENTITY** | Fold into #2 and set `displayName`. |
...
| 12 | `44422b2e1` Fix az/kubectl consent | **FOLD / #11** | Preserve final behavior in command-broker tests. |
...
| 16 | `c09a33860` Allow registration script | **DROP / CLUSTER** | The script was removed by #19; do not expose it through the broker. |
| 17 | `c79db3039` Remove kubelogin permission | **FOLD / #13** | Keep only the final least-privilege scope set. |
...
| 26 | `24889f999` Update package name and author | **FOLD / IDENTITY** | Fold into #2. |
...
| 29 | `067bf68f3` Show AKS version in top bar | **FOLD / PUBLIC** | Consume the same product-information API as #28. |
...
| 36 | `e0f08105d` Suppress az confirmation | **FOLD / #11** | Represent any product-approved scope in policy; do not hard-code AKS bypasses. |
| 37 | `d484bcd0f` Disable release notes | **CFG / PUBLIC** | Set the public `releaseNotes` product flag. |
| 38 | `0f537cf57` Add LogsViewer index | **FOLD / #30** | Include it in the LogsViewer upstream PR. |
...
| 44 | `c01009765` Update icons | **FOLD / #3** | Keep only the current AKS icon set. |
| 45 | `ceea7720d` Update Azure logo | **CFG / IDENTITY, PUBLIC** | Supply product logo assets without editing Headlamp source. |
| 46 | `bd39620b1` Reload after cluster deletion | **UP** | Submit the complementary generic deletion-state fix. |
...
| 53 | `e697c0aaf` Override macOS version | **FOLD / IDENTITY** | Use the single product version source from #27. |
...
| 56 | `5504295b8` Comment default consent | **FOLD / #11** | Document the generic policy instead of retaining a code-only commit. |
...
| 58 | `00320948b` Stop retries for 400/401 | **UP** | Submit the generic React Query retry fix with status tests. |
...
| 61 | `a73b4302a` Move `getAllowedNamespaces` | **FOLD / #59** | Move only if needed by the final namespace design; the export already exists upstream. |
| 62 | `c9ec6c99c` Update branded error snapshots | **FOLD / #55** | Regenerate product-level tests. |
...
| 64 | `cdee9d62b` Include kubelogin script conditionally | **FOLD / #54** | Resolve the declared tool only for clusters that need it. |
...
| 72 | `2f54e5cde` Fix log-search e2e test | **FOLD / #30** | Include with LogsViewer. |
...
| 80 | `da5a560ed` Move OAuth tests to Vitest | **FOLD / #79** | Include tests with the generalized provider. |
...
| 89 | `44e78118a` Fix severity narration | **FOLD / #30** | Include the accessible LogsViewer control. |
| 90 | `26d8e68de` Label log search buttons | **FOLD / #30** | Include the accessible LogsViewer control. |
...
| 104 | `e87e6d6fe` Remove hidden cell overflow | **FOLD / #102** | Duplicate/follow-up application of the same SimpleTable change; submit once. |
...
| 108 | `0aa41fb22` Fix runCmd test mocks | **FOLD / #106** | Port relevant tests to the generic broker. |
| 109 | `903360924` Expand LogsViewer workload types | **FOLD / #30** | Include the type fix with LogsViewer. |
...
| 113 | `fcad69534` Update verified binary name | **FOLD / #112** | Keep only the final generated verification expectation. |
| 114 | `badc4713b` Use `execFileSync` for list-plugins | **UP** | Submit the focused whitespace/shell-safety fix to Electron `main.ts`. |
| 115 | `c7505cce7` Canonical AI Assistant identity | **CFG / PLUGINS, CMD** | Declare the canonical plugin ID once; broker policy keys permissions by that ID. |
