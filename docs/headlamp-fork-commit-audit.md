...
| 23 | `69e20c5dd` Exclude catalog plugins | **Upstream extension + AKS configuration / Plugin bundle** | Apply [app: select an external, digest-verified plugin manifest](headlamp-upstream-patches/headlamp-upstream-external-plugin-manifest.patch) and [externalize the AKS plugin-bundle policy](headlamp-upstream-patches/headlamp-downstream-externalize-plugin-bundle.patch). Product assembly must set `HEADLAMP_BUILD_MANIFEST` before this cleanup; the selected build/runtime manifest superset replaces the packaged runtime manifest. |
