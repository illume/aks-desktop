...
Ready patches are linked in their audited commit rows, next to the matching fork cleanup and any required AKS Desktop change. Rows 68 and 92 contain ordered patch groups. Apply row 98 after row 86; row 23 after row 47; verified external tools after command capabilities; OAuth after protocol and secure storage; and cluster providers after command capabilities and verified tools. Never ship a fork cleanup without its upstream replacement.
...
| 19 | `1d1f03e58` Programmatic AKS registration | **Upstream extension + AKS plugin / Cluster registration** | Apply [app: authorize package-declared cluster providers](headlamp-upstream-patches/headlamp-upstream-cluster-provider-capabilities.patch) and [replace the positional AKS registration bridge](headlamp-upstream-patches/headlamp-downstream-replace-aks-cluster-registration.patch). After adopting the API, migrate AKS registration to the named provider. |
...
| 23 | `69e20c5dd` Exclude catalog plugins | **Upstream extension + AKS configuration / Plugin bundle** | Apply [app: select an external, digest-verified plugin manifest](headlamp-upstream-patches/headlamp-upstream-external-plugin-manifest.patch) and [externalize the AKS plugin-bundle policy](headlamp-upstream-patches/headlamp-downstream-externalize-plugin-bundle.patch). Product assembly sets `HEADLAMP_BUILD_MANIFEST`; the selected file replaces the packaged runtime manifest. |
...
| 40 | `a1d884296` Fix app builds in CI | **Fold into rows 23 and 27 / Product identity** | The [external-manifest patch](headlamp-upstream-patches/headlamp-upstream-external-plugin-manifest.patch) removes parent-repository path probing; the product build kit supplies the manifest and release version explicitly. |
...
| 106 | `fa1dcf6d1` Add runCmd plugin identification | **Upstream extension / Command execution** | Apply [app: authorize manifest-declared plugin commands](headlamp-upstream-patches/headlamp-upstream-plugin-command-capabilities.patch) and [replace package command branches with capabilities](headlamp-upstream-patches/headlamp-downstream-replace-plugin-command-capabilities.patch). Before cleanup, declare the exact AKS command scopes in the plugin manifest. |
