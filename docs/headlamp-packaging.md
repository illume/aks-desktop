...
files are generated by `build/setup-npm.ts` and never committed. The
...
Installation is also explicit. `build/setup-npm.ts` materializes the source,
then runs npm 12's `npm ci` to pack the local source directory and apply the
patch without a source-package lifecycle hook.
