...
`@headlamp-k8s/headlamp-source`. The tracked package skeleton supplies npm
scripts for web/backend, desktop application, container builds,
manifest-driven plugin bundling, and packaged-application smoke tests. Before
installation, the setup build step fetches the pinned commit and copies its
Git-tracked source into the ignored package directory. The generated package
records:
...
source build/setup-npm.sh       # materialize source and select npm 12
...
[`packages/headlamp-source`](../packages/headlamp-source); upstream Headlamp
files are generated by `build/setup-npm.sh` and never committed. The
materializer verifies the full pinned SHA, rejects a different or dirty
worktree, and copies only Git-tracked regular files. The lockfile resolves the
registry-style exact version to that generated local directory, and npm packs
it during installation before applying the consumer patch. Publishing changes
only the lockfile's `resolved` location, not the dependency selector or patch
...
Installation is also explicit. `build/setup-npm.sh` materializes the source,
then `npm ci` packs the local source directory and applies the patch but runs no
source-package lifecycle hook.
