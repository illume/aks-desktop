...
/**
 * Demonstrates smart scroll when "Agent working…" appears.
 * The viewport scrolls so the user's question stays visible with the
 * loading indicator below it. If the indicator is already visible, no
 * scroll occurs. Click "Ask Question" to add a user question and trigger
 * the agent working state with progressive thinking steps.
 */
export const AgentWorkingSmartScroll: StoryFn = () => {
  const [history, setHistory] = useState<Prompt[]>(previousConversation.slice(0, -1));
  const [isLoading, setIsLoading] = useState(false);
  ...
  const simulateThinkingSteps = (stepIndex: number) => {
    const allSteps: AgentThinkingStep[] = [
      {
        id: 1,
      ...
      {
        id: 3,
        label: 'Planning investigation tasks',
        status: 'running',
        timestamp: Date.now(),
        phase: 'planning',
      },
      ...
      {
        id: 5,
        label: 'Running kubectl get services',
        status: 'pending',
        timestamp: Date.now(),
        phase: 'executing',
      },
      {
        id: 6,
        label: 'Checking network policies',
        status: 'pending',
        timestamp: Date.now(),
        phase: 'executing',
      },
      {
        id: 7,
        label: 'Analyzing DNS configuration',
        status: 'pending',
        timestamp: Date.now(),
        phase: 'executing',
      },
      {
        id: 8,
        label: 'Inspecting ingress rules',
        status: 'pending',
        timestamp: Date.now(),
        phase: 'executing',
      },
    ...
    if (stepIndex < allSteps.length) {
      setThinkingSteps(allSteps.slice(0, stepIndex + 1));
      stepTimerRef.current = setTimeout(() => simulateThinkingSteps(stepIndex + 1), 800);
    }
  };
  ...
  const addExchange = () => {
    ...
    // Progressively add thinking steps
    setTimeout(() => simulateThinkingSteps(0), 300);
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: 500, maxWidth: 700 }}>
        <Box sx={{ flex: 1, overflow: 'hidden' }}>
          <TextStreamContainer
            ...
            isLoading={isLoading}
            apiError={null}
            agentThinkingSteps={thinkingSteps}
          />
        </Box>
        <Box sx={{ p: 1, borderTop: '1px solid', borderColor: 'divider' }}>
          <Button variant="contained" onClick={addExchange} disabled={isLoading}>
            {isLoading ? 'Agent working…' : 'Ask Question'}
          </Button>
        </Box>
      </Box>
    </ThemeProvider>
...
/**
 * Same as AgentWorkingSmartScroll but with a long YAML user question.
 * Verifies the viewport shows the bottom of the long question with the
 * "Agent working…" indicator visible below.
 */
export const AgentWorkingLongQuestionScroll: StoryFn = () => {
  const [history, setHistory] = useState<Prompt[]>([
    { role: 'user', content: 'How do I get started with AKS?' },
    {
      role: 'assistant',
...
Let me know if you need help with deployments or networking!`,
  ...
  const simulateThinkingSteps = (stepIndex: number) => {
    const allSteps: AgentThinkingStep[] = [
      ...
      {
        id: 2,
        label: 'Analyzing deployment YAML',
        status: 'running',
        timestamp: Date.now(),
        phase: 'planning',
      },
      {
        id: 3,
        label: 'Checking resource limits',
        status: 'pending',
        timestamp: Date.now(),
        phase: 'executing',
      },
  ...
  const addExchange = () => {
    setHistory(prev => [...prev, { role: 'user', content: longUserQuestions[0] }]);
    setIsLoading(true);
    setThinkingSteps([]);
    ...
    timerRef.current = setTimeout(() => {
      setHistory(prev => [...prev, { role: 'assistant', content: longQuestionResponses[0] }]);
      setIsLoading(false);
      setThinkingSteps([]);
    }, 5000);
  };
  ...
  return (
    <ThemeProvider theme={lightTheme}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: 500, maxWidth: 700 }}>
        ...
        <Box sx={{ p: 1, borderTop: '1px solid', borderColor: 'divider' }}>
          <Button variant="contained" onClick={addExchange} disabled={isLoading}>
            {isLoading ? 'Agent working…' : 'Send Long YAML Question'}
          </Button>
        </Box>
      </Box>
    </ThemeProvider>
