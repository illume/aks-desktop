...
export const ShortMessages: StoryFn = () => (
  <Box sx={{ maxWidth: 700, p: 2, bgcolor: 'background.default' }}>
    ...
    <MessageBubble sender="assistant" content="Hi! How can I help you with your AKS cluster today?" />
...
export const ErrorAndSuccessStates: StoryFn = () => (
  <Box sx={{ maxWidth: 700, p: 2, bgcolor: 'background.default' }}>
    ...
    <MessageBubble sender="assistant" content={JSON.stringify({ error: true, content: 'Connection timeout: Unable to reach the Kubernetes API server at https://myaks-12345.hcp.eastus.azmk8s.io:443' })} isError />
    <MessageBubble sender="user" content="Try again" />
    <MessageBubble sender="assistant" content={JSON.stringify({ error: true, content: 'Permission denied: The current service principal does not have RBAC permissions to scale deployments in the production namespace.' })} isError />
    <MessageBubble sender="user" content="Use the admin kubeconfig" />
    <MessageBubble sender="assistant" content={JSON.stringify({ success: true, content: 'Successfully scaled deployment "api-server" from 3 to 5 replicas in namespace "production".' })} isSuccess />
    <MessageBubble sender="assistant" content={`Scaling is complete. Here's the current status:
...
export const ErrorAndSuccessStatesDark: StoryFn = () => (
  <ThemeProvider theme={darkTheme}>
    ...
    <Box sx={{ maxWidth: 700, p: 2, bgcolor: 'background.default' }}>
      ...
      <MessageBubble sender="user" content="Use the admin kubeconfig" />
      <MessageBubble sender="assistant" content={JSON.stringify({ success: true, content: 'Successfully scaled deployment "api-server" from 3 to 5 replicas in namespace "production".' })} isSuccess />
      <MessageBubble sender="assistant" content={`Scaling is complete. All pods are running and healthy.`} />
    </Box>
  </ThemeProvider>
);
